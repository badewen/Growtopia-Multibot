...
LUA_API void lua_stopvm (lua_State* L, std::string reason) {
    L->l_G->stopvm = 1;
    L->l_G->stopreasonbuf = (char*)malloc(strlen(reason.c_str()));
    if (L->l_G->stopreasonbuf) {
        strcpy(L->l_G->stopreasonbuf, reason.c_str());
    }
}

LUA_API char lua_isvmstopped (lua_State* L) {
    return L->l_G->stopvm;
}
