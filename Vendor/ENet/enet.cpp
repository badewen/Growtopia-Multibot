...
#undef AT_HAVE_ATOMICS
    ...
    void enet_protocol_dispatch_state(ENetHost *host, ENetPeer *peer, ENetPeerState state)
    {
    ...
    void enet_protocol_notify_disconnect_timeout(ENetHost *host, ENetPeer *peer, ENetEvent *event)
    {
        if (peer->state >= ENET_PEER_STATE_CONNECTION_PENDING)
        {
            host->recalculateBandwidthLimits = 1;
    ...
    ENetProtocolCommand enet_protocol_remove_sent_reliable_command(ENetPeer *peer, enet_uint16 reliableSequenceNumber, enet_uint8 channelID)
    {
    ...
    ENetPeer *enet_protocol_handle_connect(ENetHost *host, ENetProtocolHeader *header, ENetProtocol *command)
    {
        ENET_UNUSED(header)
    ...
    int enet_protocol_handle_send_reliable(ENetHost *host, ENetPeer *peer, const ENetProtocol *command, enet_uint8 **currentData)
    {
        size_t dataLength;
    ...
    int enet_protocol_handle_send_unsequenced(ENetHost *host, ENetPeer *peer, const ENetProtocol *command, enet_uint8 **currentData)
    {
    ...
    int enet_protocol_handle_send_unreliable(ENetHost *host, ENetPeer *peer, const ENetProtocol *command,
    ...
    int enet_protocol_handle_send_fragment(ENetHost *host, ENetPeer *peer, const ENetProtocol *command, enet_uint8 **currentData)
    {
    ...
    int enet_protocol_handle_send_unreliable_fragment(ENetHost *host, ENetPeer *peer, const ENetProtocol *command, enet_uint8 **currentData)
    {
    ...
    int enet_protocol_handle_ping(ENetHost *host, ENetPeer *peer, const ENetProtocol *command)
    {
        ENET_UNUSED(host)
        ENET_UNUSED(command)
    ...
    int enet_protocol_handle_bandwidth_limit(ENetHost *host, ENetPeer *peer, const ENetProtocol *command)
    {
        if (peer->state != ENET_PEER_STATE_CONNECTED && peer->state != ENET_PEER_STATE_DISCONNECT_LATER)
        {
            return -1;
    ...
    int enet_protocol_handle_throttle_configure(ENetHost *host, ENetPeer *peer, const ENetProtocol *command)
    {
        ENET_UNUSED(host)
    ...
    int enet_protocol_handle_disconnect(ENetHost *host, ENetPeer *peer, const ENetProtocol *command)
    {
    ...
    int enet_protocol_handle_acknowledge(ENetHost *host, ENetEvent *event, ENetPeer *peer, const ENetProtocol *command)
    {
    ...
    int enet_protocol_handle_verify_connect(ENetHost *host, ENetEvent *event, ENetPeer *peer, const ENetProtocol *command)
    {
        enet_uint32 mtu, windowSize;
        size_t channelCount;
    ...
    int enet_protocol_receive_incoming_commands(ENetHost *host, ENetEvent *event)
    {
        ...
        for (packets = 0; packets < 256; ++packets)
        {
            ...
            if (host->usingProxy)
            {
                memmove(buffer.data, (char *)buffer.data + sizeof(ENetSocks5Header), receivedLength - sizeof(ENetSocks5Header));
            }
    ...
    void enet_protocol_send_unreliable_outgoing_commands(ENetHost *host, ENetPeer *peer)
    {
        ENetProtocol *command = &host->commands[host->commandCount];
        ENetBuffer *buffer = &host->buffers[host->bufferCount];
        ENetOutgoingCommand *outgoingCommand;
    ...
    int enet_protocol_check_timeouts(ENetHost *host, ENetPeer *peer, ENetEvent *event)
    {
        ENetOutgoingCommand *outgoingCommand;
    ...
    enet_protocol_send_outgoing_commands(ENetHost *host, ENetEvent *event, int checkForTimeouts)
    {
        ...
        if (host->usingNewPacket)
        {
            ...
            if (host->usingProxy)
            {
                port = ENET_HOST_TO_NET_16(host->proxyHeader.ipv4.port);
            }
    ...
    int enet_host_service(ENetHost *host, ENetEvent *event, enet_uint32 timeout)
    {
        ...
        if (host->usingProxy && host->proxyState != ENET_SOCKS5_STATE_CONNECTED)
        {
            ...
            case ENET_SOCKS5_STATE_SEND_AUTH_REQUEST:
            {
                ...
                authReq.authMethods[0] = (host->proxyInfo.auth.username == NULL || host->proxyInfo.auth.password == NULL) ? ENET_SOCKS5_AUTH_UNAUTHORIZED : ENET_SOCKS5_AUTH_USERNAME_PASSWORD;

                buffer.data = &authReq;
                buffer.dataLength = sizeof(authReq) - sizeof(authReq.authMethods) + authReq.authMethodCount;

                if (enet_socket_send(host->proxySocket, &host->proxyAddress, &buffer, 1) > 0)
            ...
            case ENET_SOCKS5_STATE_RECEIVE_AUTH_RESPONSE:
            {
                ...
                {
                    if (authRes.version != ENET_SOCKS5_VERSION_5 || (authRes.authMethod != ENET_SOCKS5_AUTH_UNAUTHORIZED && authRes.authMethod != ENET_SOCKS5_AUTH_USERNAME_PASSWORD))
                    {
            ...
            case ENET_SOCKS5_STATE_SEND_AUTH_REQUEST_USERNAME:
            {
                ...
                authReqUsername.usernameLength = strlen(host->proxyInfo.auth.username);
                strcpy((char *)&authReqUsername.username, host->proxyInfo.auth.username);

                enet_uint8 pass_len = strlen(host->proxyInfo.auth.password);
                memcpy((char *)&authReqUsername.username + authReqUsername.usernameLength, &pass_len, 1);
                strcpy((char *)&authReqUsername.username + authReqUsername.usernameLength + 1, host->proxyInfo.auth.password);

                buffer.data = &authReqUsername;
                buffer.dataLength = sizeof(authReqUsername) - 255 * 2 + authReqUsername.usernameLength + pass_len;
            ...
            case ENET_SOCKS5_STATE_RECEIVE_RESPONSE:
            {
                ...
                if (enet_socket_receive(host->proxySocket, &host->proxyAddress, &buffer, 1) > 0)
                {
                    ...
                    {
                        enet_inaddr_map4to6(*(in_addr *)&res.ipv4.addr, &host->address.host);
                        host->address.port = ENET_HOST_TO_NET_16(res.ipv4.port);
            ...
            return 0; // block enet protocol shit until proxy is connected
        }
    ...
    static void enet_packet_set_free_callback(ENetPacket *packet, void *callback)
    {
     ...
     */
    static ENetPacket *enet_peer_receive(ENetPeer *peer, enet_uint8 *channelID)
    {
    ...
    static void enet_peer_reset_queues(ENetPeer *peer)
    {
        ENetChannel *channel;
    ...
    static void enet_peer_on_connect(ENetPeer *peer)
    {
        if (peer->state != ENET_PEER_STATE_CONNECTED && peer->state != ENET_PEER_STATE_DISCONNECT_LATER)
        {
            if (peer->incomingBandwidth != 0)
    ...
    static void enet_peer_on_disconnect(ENetPeer *peer)
    {
        if (peer->state == ENET_PEER_STATE_CONNECTED || peer->state == ENET_PEER_STATE_DISCONNECT_LATER)
        {
            if (peer->incomingBandwidth != 0)
     ...
     */
    static void enet_peer_reset(ENetPeer *peer)
    {
        enet_peer_on_disconnect(peer);
     ...
     */
    static void enet_peer_ping(ENetPeer *peer)
    {
        ENetProtocol command;
     ...
     */
    static void enet_peer_ping_interval(ENetPeer *peer, enet_uint32 pingInterval)
    {
    ...
    static void enet_peer_timeout(ENetPeer *peer, enet_uint32 timeoutLimit, enet_uint32 timeoutMinimum, enet_uint32 timeoutMaximum)
    {
     ...
     *  once the disconnection is complete.
     */
    static void enet_peer_disconnect_later(ENetPeer *peer, enet_uint32 data)
    {
        if ((peer->state == ENET_PEER_STATE_CONNECTED || peer->state == ENET_PEER_STATE_DISCONNECT_LATER) &&
            !(enet_list_empty(&peer->outgoingReliableCommands) &&
              enet_list_empty(&peer->outgoingUnreliableCommands) &&
    ...
    static ENetAcknowledgement *enet_peer_queue_acknowledgement(ENetPeer *peer, const ENetProtocol *command, enet_uint16 sentTime)
    {
        ENetAcknowledgement *acknowledgement;
    ...
    static void enet_peer_setup_outgoing_command(ENetPeer *peer, ENetOutgoingCommand *outgoingCommand)
    {
    ...
    static ENetOutgoingCommand *enet_peer_queue_outgoing_command(ENetPeer *peer, const ENetProtocol *command, ENetPacket *packet, enet_uint32 offset, enet_uint16 length)
    {
    ...
    static void enet_peer_dispatch_incoming_unreliable_commands(ENetPeer *peer, ENetChannel *channel)
    {
    ...
    static void enet_peer_dispatch_incoming_reliable_commands(ENetPeer *peer, ENetChannel *channel)
    {
        ENetListIterator currentCommand;
    ...
    static ENetIncomingCommand *enet_peer_queue_incoming_command(ENetPeer *peer, const ENetProtocol *command, const void *data, size_t dataLength, enet_uint32 flags, enet_uint32 fragmentCount)
    {
    ...
    ENetHost *enet_host_create(const ENetAddress *address, size_t peerCount, size_t channelLimit, enet_uint32 incomingBandwidth, enet_uint32 outgoingBandwidth)
    {
        ...
        enet_socket_set_option(host->proxySocket, ENET_SOCKOPT_IPV6_V6ONLY, 0);
    ...
    ENetPeer *enet_host_connect(ENetHost *host, const ENetAddress *address, size_t channelCount, enet_uint32 data)
    {
        ...
        if (host->usingProxy)
        {
            enet_address_set_host(&host->proxyAddress, host->proxyInfo.ip);
            ...
            enet_inaddr_map6to4(&address->host, (in_addr *)&host->proxyHeader.ipv4.addr);
            host->proxyHeader.ipv4.port = ENET_HOST_TO_NET_16(address->port);
        }
        ...
        currentPeer->address = *address; // set address later to proxy address
     ...
     */
    static void enet_host_broadcast(ENetHost *host, enet_uint8 channelID, ENetPacket *packet)
    {
        ENetPeer *currentPeer;
     ...
     *  @sa enet_socket_send
     */
    static int enet_host_send_raw(ENetHost *host, const ENetAddress *address, enet_uint8 *data, size_t dataLength)
    {
        ENetBuffer buffer;
        buffer.data = data;
     ...
     *  @sa enet_socket_send
     */
    static int enet_host_send_raw_ex(ENetHost *host, const ENetAddress *address, enet_uint8 *data, size_t skipBytes, size_t bytesToSend)
    {
        ENetBuffer buffer;
     ...
     */
    static void enet_host_set_intercept(ENetHost *host, const ENetInterceptCallback callback)
    {
     ...
     */
    static void enet_host_compress(ENetHost *host, const ENetCompressor *compressor)
    {
        if (host->compressor.context != NULL && host->compressor.destroy)
        {
            (*host->compressor.destroy)(host->compressor.context);
     ...
     *  @param channelLimit the maximum number of channels allowed; if 0, then this is equivalent to ENET_PROTOCOL_MAXIMUM_CHANNEL_COUNT
     */
    static void enet_host_channel_limit(ENetHost *host, size_t channelLimit)
    {
        if (!channelLimit || channelLimit > ENET_PROTOCOL_MAXIMUM_CHANNEL_COUNT)
        {
            channelLimit = ENET_PROTOCOL_MAXIMUM_CHANNEL_COUNT;
     ...
     */
    static void enet_host_bandwidth_limit(ENetHost *host, enet_uint32 incomingBandwidth, enet_uint32 outgoingBandwidth)
    {
        host->incomingBandwidth = incomingBandwidth;
        host->outgoingBandwidth = outgoingBandwidth;
        host->recalculateBandwidthLimits = 1;
    ...
    static void enet_host_bandwidth_throttle(ENetHost *host)
    {
...
int clock_gettime(int X, struct timespec *ts)
{
    ...
    static enet_uint32 enet_time_get()
    {
    ...
    static void enet_inaddr_map4to6(struct in_addr in, struct in6_addr *out)
    {
    ...
    static void enet_inaddr_map6to4(const struct in6_addr *in, struct in_addr *out)
    {
    ...
    static int enet_in6addr_lookup_host(const char *name, bool nodns, ENetAddress *out)
    {
    ...
    static int enet_address_get_host_ip(const ENetAddress *address, char *name, size_t nameLength)
    {
    ...
    static int enet_address_get_host(const ENetAddress *address, char *name, size_t nameLength)
    {
...
#define NS_INT16SZ 2
    ...
    static enet_uint64 enet_host_random_seed(void)
    {
        return (enet_uint64)time(NULL);
    }

    static int enet_address_set_host_ip_old(ENetAddress *address, const char *name)
    {
    ...
    static int enet_address_set_host_old(ENetAddress *address, const char *name)
    {
        struct addrinfo hints, *resultList = NULL, *result = NULL;
    ...
    static int enet_address_get_host_ip_old(const ENetAddress *address, char *name, size_t nameLength)
    {
        if (inet_ntop(AF_INET6, &address->host, name, nameLength) == NULL)
        {
            return -1;
    ...
    static int enet_address_get_host_old(const ENetAddress *address, char *name, size_t nameLength)
    {
        struct sockaddr_in6 sin;
        int err;
        memset(&sin, 0, sizeof(struct sockaddr_in6));
    ...
    static int enet_socket_bind(ENetSocket socket, const ENetAddress *address)
    {
        struct sockaddr_in6 sin;
        memset(&sin, 0, sizeof(struct sockaddr_in6));
        sin.sin6_family = AF_INET6;
    ...
    static int enet_socket_get_address(ENetSocket socket, ENetAddress *address)
    {
        struct sockaddr_in6 sin;
    ...
    static int enet_socket_listen(ENetSocket socket, int backlog)
    {
    ...
    static int enet_socket_set_option(ENetSocket socket, ENetSocketOption option, int value)
    {
        int result = -1;
    ...
    static int enet_socket_get_option(ENetSocket socket, ENetSocketOption option, int *value)
    {
        int result = -1;
        socklen_t len;
    ...
    static int enet_socket_connect(ENetSocket socket, const ENetAddress *address)
    {
        struct sockaddr_in6 sin;
        int result;
    ...
    static int enet_socket_shutdown(ENetSocket socket, ENetSocketShutdown how)
    {
        return shutdown(socket, (int)how);
    }

    static void enet_socket_destroy(ENetSocket socket)
    {
        if (socket != -1)
        {
            close(socket);
    ...
    static int enet_socket_send(ENetSocket socket, const ENetAddress *address, const ENetBuffer *buffers, size_t bufferCount)
    {
    ...
    static int enet_socket_receive(ENetSocket socket, ENetAddress *address, ENetBuffer *buffers, size_t bufferCount)
    {
        struct msghdr msgHdr;
        struct sockaddr_in6 sin;
        int recvLength;
    ...
    static int enet_socketset_select(ENetSocket maxSocket, ENetSocketSet *readSet, ENetSocketSet *writeSet, enet_uint32 timeout)
    {
        struct timeval timeVal;
    ...
    static int enet_socket_wait(ENetSocket socket, enet_uint32 *condition, enet_uint64 timeout)
    {
    ...
    static void enet_deinitialize(void)
    {
        timeEndPeriod(1);
        WSACleanup();
    }
    ...
    static ENetSocket enet_socket_create(ENetSocketType type)
    {
        return socket(PF_INET6, type == ENET_SOCKET_TYPE_DATAGRAM ? SOCK_DGRAM : SOCK_STREAM, 0);
    }
    ...
    static ENetSocket enet_socket_accept(ENetSocket socket, ENetAddress *address)
    {
        SOCKET result;
        struct sockaddr_in6 sin;
        int sinLength = sizeof(struct sockaddr_in6);
...
#define ENET_CONTEXT_ENCODE_EXCLUDE(context, value_, under, total, minimum)              \
    ...
    static size_t enet_range_coder_compress(void *context, const ENetBuffer *inBuffers, size_t inBufferCount, size_t inLimit, enet_uint8 *outData, size_t outLimit)
    {
        ENetRangeCoder *rangeCoder = (ENetRangeCoder *)context;
